...
run_docker_command(f"docker exec namenode mkdir -p {container_script_dir}")

# Step 2: Copy mapper and reducer scripts to container
print(f"Copying mapper script from {mapper_script_host_path} to namenode:{container_mapper_path}...")
if not os.path.exists(mapper_script_host_path):
    print(f"Mapper script not found at {mapper_script_host_path}. Exiting.")
    exit(1)
if not run_docker_command(f"docker cp \"{mapper_script_host_path}\" namenode:{container_mapper_path}"):
    print(f"Failed to copy mapper script. Exiting.")
    exit(1)

print(f"Copying reducer script from {reducer_script_host_path} to namenode:{container_reducer_path}...")
if not os.path.exists(reducer_script_host_path):
    print(f"Reducer script not found at {reducer_script_host_path}. Exiting.")
    exit(1)
if not run_docker_command(f"docker cp \"{reducer_script_host_path}\" namenode:{container_reducer_path}"):
...
print(f"Writing MapReduce bash script to namenode:{bash_script_in_container_path}...")
if not run_docker_command(f'docker exec -i namenode bash -c "cat > {bash_script_in_container_path}"',
                          input_data=mapreduce_bash_script_content.encode()):
...
if not run_docker_command(f"docker exec namenode chmod +x {bash_script_in_container_path}"):
...
if not run_docker_command(f"docker exec namenode {bash_script_in_container_path}"):
...
print(f"Attempting to copy results from namenode:{container_output_file} to {local_output_file}...")
if run_docker_command(f"docker cp namenode:{container_output_file} \"{local_output_file}\""):
    ...
    run_docker_command(f"docker exec namenode ls -l /tmp/mapreduce_output/")
